...
<body onload="updateClock(); setInterval('updateClock()', 1000); updateDate(); /*changeRoom();*/">
	...
	<div class="whole_screen">
		...
		<?php
			...
			$sql = "SELECT * FROM display.reservation WHERE active=1 AND ABS(roomNumber - $room) < 0.001 AND startDateTime<NOW() AND endDateTime>NOW()";
			...
			if ($result->num_rows > 0) {
				...
				$until_sql = "SELECT * FROM display.reservation WHERE active=1 AND ABS(roomNumber - $room) < 0.001 AND startDateTime = (SELECT MIN(startDateTime) FROM display.reservation WHERE active=1 AND ABS(roomNumber - $room) < 0.001)";
